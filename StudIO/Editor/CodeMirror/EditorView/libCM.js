(function (global, factory) {
	...
	var char_1 = createCommonjsModule(function (module, exports) {
	  ...
	  } catch (_) {} // FIXME this doesn't work for astral chars yet (need different calling convention)
	...
	var column = createCommonjsModule(function (module, exports) {
	  ...
	  function countColumn(string, n, tabSize) {
	    for (var i = 0; i < string.length; i++) {
	      ...
	      if (code == 9) n += tabSize - n % tabSize;else if (code < 768 || !char_1.isExtendingChar(string.charAt(i))) n++;
	    }
	  ...
	  function findColumn(string, n, col, tabSize) {
	    for (var i = 0; i < string.length; i++) {
	      var code = string.charCodeAt(i);
	      if (code >= 768 && char_1.isExtendingChar(string.charAt(i))) continue;
	      if (n >= col) return {
	        offset: i,
	        leftOver: 0
	      };
	...
	var text = createCommonjsModule(function (module, exports) {
	  ...
	  var BASE_LEAF = 512; // The max size of a leaf node

	  var MAX_LEAF = BASE_LEAF << 1; // The desired amount of branches per node, as an exponent of 2 (so 3
	  // means 8 branches)

	  var TARGET_BRANCH_SHIFT = 3; // Note line numbers are 1-based
	  ...
	  function () {
	    ...
	    Text.prototype.eq = function (other) {
	    ...
	    Text.of = function (text, lineSeparator) {
	      if (typeof text == "string") text = splitLines(text, lineSeparator);else if (text.length == 0) throw new RangeError("A document must have at least one line");
	  ...
	  function (_super) {
	    ...
	    TextLeaf.prototype.lastLineLength = function () {
	      return this.text[this.text.length - 1].length;
	    };

	    TextLeaf.prototype.firstLineLength = function () {
	  ...
	  function (_super) {
	    ...
	    TextNode.prototype.replaceInner = function (from, to, text, length) {
	      var lengthDiff = length - (to - from),
	          newLength = this.length + lengthDiff;
	      if (newLength <= BASE_LEAF) return new TextLeaf(appendText(this.sliceLines(to), appendText(text, this.sliceTo(0, from, [""]))), newLength);
	      var children;
	      ...
	      for (var i = 0, pos = 0; i < this.children.length; i++) {
	        ...
	        if (from >= pos && to <= end && (lengthDiff > 0 ? child.length + lengthDiff < Math.max(newLength >> TARGET_BRANCH_SHIFT - 1, MAX_LEAF) : child.length + lengthDiff > newLength >> TARGET_BRANCH_SHIFT + 1)) {
	          ...
	          if (pos < from) {
	            if (end == from) children.push(child);else child.decomposeStart(from - pos, children);
	          }

	          if (pos <= from && end >= from) TextLeaf.split(text, children);
	          if (pos >= to) children.push(child);else if (end > to) child.decomposeEnd(to - pos, children);
	        }
	    ...
	    TextNode.prototype.sliceTo = function (from, to, target) {
	      ...
	      for (var _i = 0, _a = this.children; _i < _a.length; _i++) {
	        ...
	        if (to > pos && from < end) child.sliceTo(Math.max(0, from - pos), Math.min(child.length, to - pos), target);
	        pos = end;
	      }
	    ...
	    TextNode.prototype.lineInner = function (target, isLine, line, offset) {
	      for (var i = 0;; i++) {
	        ...
	        if ((isLine ? endLine : end) >= target) {
	          var inner = child.lineInner(target, isLine, line, offset),
	              add = void 0;
	    ...
	    TextNode.prototype.decomposeEnd = function (from, target) {
	      ...
	      for (var _i = 0, _a = this.children; _i < _a.length; _i++) {
	        var child = _a[_i];
	        var end = pos + child.length;
	        if (pos >= from) target.push(child);else if (end > from && pos < from) child.decomposeEnd(from - pos, target);
	        pos = end;
	      }
	    };
	    ...
	    TextNode.prototype.lastLineLength = function () {
	    ...
	    TextNode.prototype.firstLineLength = function () {
	    ...
	    TextNode.from = function (children, length) {
	      ...
	      var chunkLength = Math.max(BASE_LEAF, length >> TARGET_BRANCH_SHIFT),
	      ...
	      function add(child) {
	        ...
	        if (childLength > maxLength && child instanceof TextNode) {
	          ...
	          flush();
	          chunked.push(child);
	        } else if (child instanceof TextLeaf && currentLength > 0 && (last = currentChunk[currentChunk.length - 1]) instanceof TextLeaf && child.length + last.length <= BASE_LEAF) {
	  ...
	  function eqContent(a, b) {
	    ...
	    for (var offA = 0, offB = 0;;) {
	      if (iterA.lineBreak != iterB.lineBreak || iterA.done != iterB.done) {
	        ...
	        var strA = iterA.value.slice(offA),
	    ...
	    RawTextCursor.prototype.next = function (skip) {
	      ...
	      for (;;) {
	        ...
	        if (top_1 instanceof TextLeaf) {
	          ...
	          var next = top_1.children[this.dir > 0 ? offset : offset - 1],
	              len = next.length;
	          this.offsets[last] = offset + this.dir;
	  ...
	  function () {
	    ...
	    PartialTextCursor.prototype.next = function () {
	      if (this.limit <= 0) {
	        ...
	        var _a = this.cursor.next(this.skip),
	            value = _a.value,
	  ...
	  /** @class */
	  function () {
	    function Line(start, end, number, // @internal
	    content) {
	      this.start = start;
	      this.end = end;
	      this.number = number;
	    ...
	    Line.prototype.slice = function (from, to) {
	      ...
	      if (typeof this.content == "string") return to == from + 1 ? this.content.charAt(from) : this.content.slice(from, to);
	      if (from == to) return "";
	...
	var selection = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    SelectionRange.prototype.map = function (mapping) {
	      var anchor = mapping.mapPos(this.anchor),
	          head = mapping.mapPos(this.head);
	      if (anchor == this.anchor && head == this.head) return this;else return new SelectionRange(anchor, head);
	    };
	    ...
	    SelectionRange.prototype.toJSON = function () {
	      return this;
	    };
	    ...
	    SelectionRange.groupAt = function (state, pos, bias) {
	      ...
	      var line = state.doc.lineAt(pos),
	          linePos = pos - line.start;
	      if (line.length == 0) return new SelectionRange(pos);
	      if (linePos == 0) bias = 1;else if (linePos == line.length) bias = -1;
	      var read = linePos + (bias < 0 ? -1 : 0),
	          type = src.charType(line.slice(read, read + 1));
	      var from = pos,
	          to = pos;

	      for (var lineFrom = linePos; lineFrom > 0 && src.charType(line.slice(lineFrom - 1, lineFrom)) == type; lineFrom--) from--;

	      for (var lineTo = linePos; lineTo < line.length && src.charType(line.slice(lineTo, lineTo + 1)) == type; lineTo++) to++;
	  ...
	  function () {
	    ...
	    EditorSelection.fromJSON = function (json) {
	      if (json && Array.isArray(json.ranges)) {
	        if (typeof json.primaryIndex != "number" || json.primaryIndex >= json.ranges.length) throw new RangeError("Invalid JSON representation for EditorSelection");
	...
	var extension = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    Slot.get = function (type, slots) {
	      for (var i = slots.length - 1; i >= 0; i--) if (slots[i].type == type) return slots[i].value;
	  ...
	  function () {
	    ...
	    Extension.prototype.fallback = function () {
	      return this.setPrio(-1);
	    };

	    Extension.prototype.extend = function () {
	      return this.setPrio(1);
	    };

	    Extension.prototype.override = function () {
	      return this.setPrio(2);
	    }; // @internal
	    ...
	    Extension.resolve = function (extensions) {
	      var pending = new this(1
	      /* Multi */
	      , null, extensions).flatten(0); // This does a crude topological ordering to resolve behaviors
	      ...
	      for (var resolved = [];;) {
	        ...
	        if (!top_1) break; // Only behaviors left
	        ...
	        if (resolved.indexOf(top_1) > -1) return this.resolve(extensions);
	      ...
	      for (var _i = 0, pending_1 = pending; _i < pending_1.length; _i++) {
	        ...
	        for (var _a = 0, pending_2 = pending; _a < pending_2.length; _a++) {
	          var e = pending_2[_a];
	          if (e.id == ext.id) e.collect(values);
	        }

	        store.behaviors.push(ext.id);
	        store.values.push(values.map(function (v) {
	          return v.value;
	        }));
	      }
	  ...
	  function () {
	    ...
	    UniqueExtensionType.prototype.resolve = function (extensions) {
	      ...
	      for (var _i = 0, extensions_1 = extensions; _i < extensions_1.length; _i++) {
	        ...
	        if (ext.id == this) ext.collect(ours);
	      }
	      ...
	      for (var i = 0; i < extensions.length; i++) {
	        ...
	        var sub = first ? this.subs(ours.map(function (s) {
	          return s.value;
	        }), ext.priority) : none;
	    ...
	    UniqueExtensionType.prototype.subs = function (specs, priority) {
	      ...
	      for (var _i = 0, subs_1 = subs; _i < subs_1.length; _i++) {
	        ...
	        if (sub.kind == 2
	        /* Unique */
	        && this.knownSubs.indexOf(sub.id) == -1) this.knownSubs.push(sub.id);
	      }
	  ...
	  var none = []; // An instance of this is part of EditorState and stores the behaviors
	  ...
	  function () {
	    function BehaviorStore() {
	      // @internal
	      this.behaviors = []; // @internal

	      this.values = []; // Any extensions that weren't an instance of the given type when
	      // resolving.
	  ...
	  exports.BehaviorStore = BehaviorStore; // Find the extension type that must be resolved next, meaning it is
	  ...
	  function fillConfig(config, defaults) {
	    ...
	    for (var key in config) result[key] = config[key];
	...
	var change = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    Object.defineProperty(ChangeDesc.prototype, "invertedDesc", {
	      get: function () {
	        return new ChangeDesc(this.from, this.from + this.length, this.to - this.from);
	      },
	      enumerable: true,
	      configurable: true
	    });
	    ...
	    ChangeDesc.prototype.toJSON = function () {
	      return this;
	    };

	    ChangeDesc.fromJSON = function (json) {
	      if (!json || typeof json.from != "number" || typeof json.to != "number" || typeof json.length != "number") throw new RangeError("Invalid JSON representation for ChangeDesc");
	  ...
	  function (_super) {
	    ...
	    Change.prototype.map = function (mapping) {
	      var from = mapping.mapPos(this.from, 1),
	          to = mapping.mapPos(this.to, -1);
	      return from > to ? null : new Change(from, to, this.text);
	    };

	    Object.defineProperty(Change.prototype, "desc", {
	      get: function () {
	        return new ChangeDesc(this.from, this.to, this.length);
	      },
	      enumerable: true,
	      configurable: true
	    });
	    ...
	    Change.fromJSON = function (json) {
	      if (!json || typeof json.from != "number" || typeof json.to != "number" || !Array.isArray(json.text) || json.text.length == 0 || json.text.some(function (val) {
	  ...
	  function () {
	    ...
	    ChangeSet.prototype.appendSet = function (changes) {
	      ...
	      return changes.length == 0 ? this : this.length == 0 ? changes : new ChangeSet(this.changes.concat(changes.changes), this.mirror.concat(changes.mirror.map(function (i) {
	        return i + _this.length;
	      })));
	    };
	    ...
	    ChangeSet.prototype.mapInner = function (pos, bias, mode, fromI, toI) {
	      ...
	      for (var i = fromI - (dir < 0 ? 1 : 0), endI = toI - (dir < 0 ? 1 : 0); i != endI; i += dir) {
	        ...
	        if (recoverables && (rec = recoverables[i]) != null) {
	          // There's a recovery for this change, and it applies
	          pos = from + rec;
	          continue;
	        }

	        if (hasMirrors && (mirror = this.getMirror(i)) != null && (dir > 0 ? mirror > i && mirror < toI : mirror < i && mirror >= toI)) {
	          // A mirror exists
	          if (pos > from && pos < to) {
	            // If this change deletes the position, skip forward to the mirror
	            i = mirror;
	    ...
	    ChangeSet.prototype.changedRanges = function () {
	      ...
	      for (var i = 0; i < this.length; i++) {
	        ...
	        var fromA = change.from,
	            toA = change.to,
	            fromB = change.from,
	            toB = change.from + change.length;
	    ...
	    Object.defineProperty(ChangeSet.prototype, "desc", {
	      get: function () {
	        ...
	        return new ChangeSet(this.changes.map(function (ch) {
	          return ch.desc;
	        }), this.mirror);
	      },
	      enumerable: true,
	    ...
	    ChangeSet.prototype.toJSON = function () {
	      var changes = this.changes.map(function (change) {
	        return change.toJSON();
	      });
	      return this.mirror.length == 0 ? changes : {
	        mirror: this.mirror,
	        changes: changes
	      };
	    };
	    ...
	    ChangeSet.fromJSON = function (ChangeType, json) {
	      ...
	      return new ChangeSet(changes.map(function (ch) {
	        return ChangeType.fromJSON(ch);
	      }), mirror);
	    };
	  ...
	  function () {
	    ...
	    Object.defineProperty(ChangedRange.prototype, "lenDiff", {
	      get: function () {
	        return this.toB - this.fromB - (this.toA - this.fromA);
	      },
	      enumerable: true,
	      configurable: true
	    });
	...
	var transaction = createCommonjsModule(function (module, exports) {
	  ...
	  var FLAG_SELECTION_SET = 1,
	      FLAG_SCROLL_INTO_VIEW = 2;
	  ...
	  function () {
	    ...
	    Transaction.prototype.change = function (change, mirror) {
	      ...
	      if (change.from < 0 || change.to < change.from || change.to > this.doc.length) throw new RangeError("Invalid change " + change.from + " to " + change.to);
	    ...
	    Transaction.prototype.forEachRange = function (f) {
	      ...
	      for (var _i = 0, _a = sel.ranges; _i < _a.length; _i++) {
	        ...
	        if (this.changes.length > before) {
	          ...
	          for (var i = 0; i < newRanges.length; i++) newRanges[i] = newRanges[i].map(mapping);
	        }
	    ...
	    Transaction.prototype.invertedChanges = function () {
	      if (!this.changes.length) return change.ChangeSet.empty;
	      var changes = [],
	          set = this.changes;

	      for (var i = set.length - 1; i >= 0; i--) changes.push(set.changes[i].invert(i == 0 ? this.startState.doc : this.docs[i - 1]));

	      return new change.ChangeSet(changes, set.mirror.length ? set.mirror.map(function (i) {
	        return set.length - i - 1;
	      }) : set.mirror);
	    };
	...
	var state = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    Configuration.create = function (config) {
	      ...
	      return new Configuration(behavior, behavior.get(stateFieldBehavior), behavior.get(StateExtension.allowMultipleSelections).some(function (x) {
	        return x;
	      }), config.tabSize || 4, config.lineSeparator || null);
	    };
	  ...
	  function () {
	    ...
	    EditorState.prototype.applyTransaction = function (tr) {
	      ...
	      var tabSize = tr.getMeta(transaction.Transaction.changeTabSize),
	          lineSep = tr.getMeta(transaction.Transaction.changeLineSeparator);
	      if (tabSize !== undefined) $conf = $conf.updateTabSize(tabSize); // FIXME changing the line separator might involve rearranging line endings (?)

	      if (lineSep !== undefined) $conf = $conf.updateLineSeparator(lineSep);
	      var fields = [];
	      var newState = new EditorState($conf, fields, tr.doc, tr.selection);

	      for (var i = 0; i < this.fields.length; i++) fields[i] = $conf.fields[i].apply(tr, this.fields[i], newState);
	    ...
	    Object.defineProperty(EditorState.prototype, "multipleSelections", {
	      get: function () {
	        return this.config.multipleSelections;
	      },
	      enumerable: true,
	      configurable: true
	    });

	    EditorState.prototype.joinLines = function (text) {
	      return src.joinLines(text, this.config.lineSeparator || undefined);
	    };

	    EditorState.prototype.splitLines = function (text) {
	      return src.splitLines(text, this.config.lineSeparator || undefined);
	    };
	    ...
	    EditorState.fromJSON = function (json, config) {
	      ...
	      if (!json || json.lineSeparator && typeof json.lineSeparator != "string" || typeof json.tabSize != "number" || typeof json.doc != "string") throw new RangeError("Invalid JSON representation for EditorState");
	    ...
	    EditorState.create = function (config) {
	      ...
	      var doc = config.doc instanceof src.Text ? config.doc : src.Text.of(config.doc || "", config.lineSeparator || undefined);
	      var selection$1 = config.selection || selection.EditorSelection.default;
	      if (!$config.multipleSelections) selection$1 = selection$1.asSingle();
	      var fields = [];
	...
	var COUNT = sym("\u037c"),
	      SET = sym("styleSet", 1),
	      DATA = sym("data", 1);
	var top = typeof global == "undefined" ? window : global; // :: (Object<Style>, number, ?{priority: ?number}) → Object<string>
	...
	class StyleSet {
	  ...
	  mount(rules, priority) {
	    ...
	    if (sheet) {
	      for (var i = 0; i < rules.length; i++) sheet.insertRule(rules[i], pos++);
	    } else {
	      this.styleTag.textContent = this.rules.join("\n");
	    }

	    for (var i = priority; i < this.insertPos.length; i++) this.insertPos[i] += rules.length;
	  }
	...
	var browser = createCommonjsModule(function (module, exports) {
	  ...
	  var _a = typeof navigator != "undefined" ? [navigator, document] : [{
	    userAgent: "",
	    vendor: "",
	    platform: ""
	  }, {
	...
	var dom = createCommonjsModule(function (module, exports) {
	  ...
	  function selectionCollapsed(domSel) {
	    ...
	    if (collapsed && browser.default.chrome && domSel.rangeCount && !domSel.getRangeAt(0).collapsed) collapsed = false;
	    return collapsed;
	  }
	  ...
	  exports.clientRectsFor = clientRectsFor; // Scans forward and backward through DOM positions equivalent to the
	  ...
	  function scanFor(node, off, targetNode, targetOff, dir) {
	    for (;;) {
	      ...
	      if (off == (dir < 0 ? 0 : maxOffset(node))) {
	        ...
	        if (!parent_1 || parent_1.nodeType != 1) return false;
	  ...
	  function scrollRectIntoView(dom, rect) {
	    ...
	    if (prev && getComputedStyle(prev).position == "sticky") gutterCover = dom.offsetLeft;

	    for (var cur = dom.parentNode; cur;) {
	      if (cur.nodeType == 1) {
	        // Element or document
	        var bounding = void 0,
	            top_1 = cur == document.body;
	        ...
	        if (top_1) {
	          ...
	          bounding = {
	            left: rect_1.left,
	            right: rect_1.left + cur.clientWidth,
	            top: rect_1.top,
	            bottom: rect_1.top + cur.clientHeight
	          };
	        }

	        var moveX = 0,
	            moveY = 0;
	        if (rect.top < bounding.top + scrollThreshold) moveY = -(bounding.top - rect.top + scrollMargin);else if (rect.bottom > bounding.bottom - scrollThreshold) moveY = rect.bottom - bounding.bottom + scrollMargin;
	        if (rect.left < bounding.left + gutterCover + scrollThreshold) moveX = -(bounding.left + gutterCover - rect.left + scrollMargin);else if (rect.right > bounding.right - scrollThreshold) moveX = rect.right - bounding.right + scrollMargin;
	...
	var contentview = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    DOMPos.before = function (dom$1, precise) {
	      return new DOMPos(dom$1.parentNode, dom.domIndex(dom$1), precise);
	    };

	    DOMPos.after = function (dom$1, precise) {
	      return new DOMPos(dom$1.parentNode, dom.domIndex(dom$1) + 1, precise);
	    };
	  ...
	  function () {
	    ...
	    ContentView.prototype.sync = function () {
	      ...
	      /* Node */
	          ...
	          var parent_1 = this.dom,
	              pos = parent_1.firstChild;
	          ...
	          for (var _i = 0, _a = this.children; _i < _a.length; _i++) {
	            ...
	            if (child.dirty) {
	              if (pos && !child.dom && !pos.cmView) {
	                ...
	                if (child.reuseDOM(pos)) pos = prev ? prev.nextSibling : parent_1.firstChild;
	              }
	    ...
	    ContentView.prototype.reuseDOM = function (dom) {
	      return false;
	    };
	    ...
	    ContentView.prototype.localPosFromDOM = function (node, offset) {
	      ...
	      if (node == this.dom) {
	        ...
	        for (;;) {
	          ...
	          if (parent_2 == this.dom) break;

	          if (bias == 0 && parent_2.firstChild != parent_2.lastChild) {
	            if (node == parent_2.firstChild) bias = -1;else bias = 1;
	          }
	    ...
	    ContentView.prototype.replaceChildren = function (from, to, children) {
	      ...
	      for (var i = from; i < to; i++) this.children[i].parent = null;

	      (_a = this.children).splice.apply(_a, [from, to - from].concat(children));

	      for (var i = 0; i < children.length; i++) children[i].setParent(this);
	    };

	    ContentView.prototype.ignoreMutation = function (rec) {
	      return false;
	    };

	    ContentView.prototype.ignoreEvent = function (event) {
	      return false;
	    };
	  ...
	  exports.ChildCursor = ChildCursor; // FIXME merge back with ChildCursor again
	  ...
	  function (_super) {
	    ...
	    function DocChildCursor(children, pos, i) {
	      ...
	      if (i) _this = _super.call(this, children, pos - children[i - 1].length, i - 1) || this;else _this = _super.call(this, children, pos, i) || this;
	      return _this;
	    }
	...
	var attributes = createCommonjsModule(function (module, exports) {
	  ...
	  function combineAttrs(source, target) {
	    for (var name_1 in source) {
	      if (name_1 == "class" && target.class) target.class += " " + source.class;else if (name_1 == "style" && target.style) target.style += ";" + source.style;else target[name_1] = source[name_1];
	    }
	  ...
	  function updateAttrs(dom, prev, attrs) {
	    if (prev) for (var name_2 in prev) if (!(attrs && name_2 in attrs)) dom.removeAttribute(name_2);
	    if (attrs) for (var name_3 in attrs) if (!(prev && prev[name_3] == attrs[name_3])) dom.setAttribute(name_3, attrs[name_3]);
	  }
	...
	var inlineview = createCommonjsModule(function (module, exports) {
	  ...
	  function (_super) {
	    ...
	    InlineView.prototype.match = function (other) {
	      return false;
	    };
	    ...
	    InlineView.prototype.getSide = function () {
	      return 0;
	    };
	  ...
	  function (_super) {
	    ...
	    TextView.prototype.createDOM = function (textDOM) {
	      ...
	      if (tagName) {
	        ...
	        if (this.attrs) for (var name_1 in this.attrs) dom.setAttribute(name_1, this.attrs[name_1]);
	        this.setDOM(dom);
	      } else {
	    ...
	    TextView.prototype.merge = function (from, to, source) {
	      ...
	      if (source && (!(source instanceof TextView) || source.tagName != this.tagName || source.class != this.class || !attributes.attrsEq(source.attrs, this.attrs) || this.length - (to - from) + source.length > MAX_JOIN_LEN)) return false;
	    ...
	    TextView.prototype.domAtPos = function (pos) {
	      return new contentview.DOMPos(this.textDOM, pos);
	    };
	  ...
	  function (_super) {
	    ...
	    WidgetView.prototype.getSide = function () {
	      return this.side;
	    };
	    ...
	    WidgetView.prototype.merge = function (from, to, source) {
	      ...
	      if (source) {
	        if (!(source instanceof WidgetView) || !source.open || from > 0 && !(source.open & 1
	        /* Start */
	        ) || to < this.length && !(source.open & 2
	        /* End */
	        )) return false;
	        if (!this.widget.compare(source.widget)) throw new Error("Trying to merge incompatible widgets");
	      }
	    ...
	    WidgetView.prototype.ignoreMutation = function () {
	      return true;
	    };

	    WidgetView.prototype.ignoreEvent = function (event) {
	    ...
	    WidgetView.prototype.domBoundsAround = function () {
	      return null;
	    };
	  ...
	  function (_super) {
	    ...
	    CompositionView.prototype.domAtPos = function (pos) {
	      return new contentview.DOMPos(this.widget.value.text, pos);
	    };
	    ...
	    CompositionView.prototype.ignoreMutation = function () {
	      return false;
	    };
	    ...
	    CompositionView.prototype.coordsAt = function (pos) {
	      return textCoords(this.widget.value.text, pos);
	    };
	...
	var blockview = createCommonjsModule(function (module, exports) {
	  ...
	  function (_super) {
	    ...
	    LineView.prototype.merge = function (from, to, source, takeDeco) {
	      ...
	      var _a = cur.findPos(to, 1),
	          toI = _a.i,
	          toOff = _a.off;

	      var _b = cur.findPos(from, -1),
	          fromI = _b.i,
	          fromOff = _b.off;
	      ...
	      this.length += dLen; // Both from and to point into the same text view

	      if (fromI == toI && fromOff) {
	        var start = this.children[fromI]; // Maybe just update that view and be done
	        ...
	        if (after.merge(0, 0, elts[elts.length - 1])) elts[elts.length - 1] = after;else elts.push(after);
	        toI++;
	        toOff = 0;
	      ...
	      if (toOff) {
	        ...
	        if (elts.length && end.merge(0, toOff, elts[elts.length - 1])) elts.pop();else end.merge(0, toOff, null);
	      } else if (toI < this.children.length && elts.length && this.children[toI].merge(0, 0, elts[elts.length - 1])) {
	        elts.pop();
	      }

	      if (fromOff) {
	        var start = this.children[fromI];
	        if (elts.length && start.merge(fromOff, undefined, elts[0])) elts.shift();else start.merge(fromOff, undefined, null);
	        fromI++;
	    ...
	    LineView.prototype.split = function (at) {
	      ...
	      var _a = this.childCursor().findPos(at),
	          i = _a.i,
	          off = _a.off;
	      ...
	      for (var j = i; j < this.children.length; j++) end.append(this.children[j]);
	    ...
	    LineView.prototype.addLineDeco = function (deco) {
	      ...
	      if (attrs) this.attrs = attributes.combineAttrs(attrs, this.attrs || {});
	    };
	    ...
	    LineView.prototype.domAtPos = function (pos) {
	      ...
	      for (; i > 0; i--) {
	        ...
	        if (before.parentNode == this.dom) return contentview.DOMPos.after(before);
	      }
	    ...
	    LineView.prototype.measureTextSize = function () {
	      ...
	      for (var _i = 0, _a = this.children; _i < _a.length; _i++) {
	        var child = _a[_i];
	        if (!(child instanceof inlineview.TextView)) return null;
	    ...
	    LineView.prototype.match = function (other) {
	      return false;
	    };
	  ...
	  function (_super) {
	    ...
	    function BlockWidgetView(widget, length, type, // This is set by the builder and used to distinguish between
	    ...
	    BlockWidgetView.prototype.merge = function (from, to, source) {
	      if (!(source instanceof BlockWidgetView) || !source.open || from > 0 && !(source.open & 1
	      /* Start */
	      ) || to < this.length && !(source.open & 2
	      /* End */
	      )) return false;
	      if (!this.widget.compare(source.widget)) throw new Error("Trying to merge an open widget with an incompatible node");
	    ...
	    BlockWidgetView.prototype.domBoundsAround = function () {
	      return null;
	    };
	...
	var rangeset = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    RangeValue.prototype.eq = function (other) {
	      return this == other;
	    };
	  ...
	  function maybeNone(array) {
	    return array.length ? array : none;
	  }

	  var BASE_NODE_SIZE_SHIFT = 5,
	      BASE_NODE_SIZE = 1 << BASE_NODE_SIZE_SHIFT;

	  var RangeSet =
	  /** @class */
	  function () {
	    // @internal
	    function RangeSet( // @internal The text length covered by this set
	    length, // The number of ranges in the set
	    size, // @internal The locally stored ranges—which are all of them
	    // for leaf nodes, and the ones that don't fit in child sets for
	    // non-leaves. Sorted by start position, then side.
	    local, // @internal The child sets, in position order. Their total
	    ...
	    RangeSet.prototype.update = function (added, filter, filterFrom, filterTo) {
	      ...
	      var maxLen = added.reduce(function (l, d) {
	        return Math.max(l, d.to);
	      }, this.length);
	    ...
	    RangeSet.prototype.updateInner = function (added, filter, filterFrom, filterTo, offset, length) {
	      ...
	      var local = filterRanges(this.local, filter, filterFrom, filterTo, offset); // The new array of child sets, if changed

	      var children = null;
	      var size = 0;
	      var decI = 0,
	          pos = offset; // Iterate over the child sets, applying filters and pushing added
	      // ranges into them

	      for (var i = 0; i < this.children.length; i++) {
	        var child = this.children[i];
	        var endPos = pos + child.length,
	            localRanges = null;
	        ...
	        var newChild = child;
	        if (localRanges || filter && filterFrom <= endPos && filterTo >= pos) newChild = newChild.updateInner(localRanges || none, filter, filterFrom, filterTo, pos, newChild.length);
	        if (newChild != child) (children || (children = this.children.slice(0, i))).push(newChild);else if (children) children.push(newChild);
	        size += newChild.size;
	        pos = endPos;
	      } // If nothing was actually updated, return the existing object


	      if (!local && !children && decI == added.length) return this; // Compute final size

	      size += (local || this.local).length + added.length - decI; // This is a small node—turn it into a flat leaf

	      if (size <= BASE_NODE_SIZE) return collapseSet(children || this.children, local || this.local.slice(), added, decI, offset, length);
	    ...
	    RangeSet.prototype.mapInner = function (changes, oldStart, newStart, newEnd) {
	      ...
	      for (var i = 0; i < this.local.length; i++) {
	        var range = this.local[i],
	            mapped = range.map(changes, oldStart, newStart);
	        var escape_1 = mapped != null && (mapped.from < 0 || mapped.to > newLength);
	        if (newLocal == null && (range != mapped || escape_1)) newLocal = this.local.slice(0, i);
	        if (escape_1) (escaped || (escaped = [])).push(mapped);else if (newLocal && mapped) newLocal.push(mapped);
	      }
	      ...
	      for (var i = 0, oldPos = oldStart, newPos = newStart; i < this.children.length; i++) {
	        ...
	        /* Yes */
	            ...
	            newChild = inner.set;
	            if (inner.escaped) for (var _i = 0, _a = inner.escaped; _i < _a.length; _i++) {
	              var range = _a[_i];
	              range = range.move(newPos - newStart);
	              if (range.from < 0 || range.to > newLength) insertSorted(escaped || (escaped = []), range);else insertSorted(newLocal || (newLocal = this.local.slice()), range);
	            }
	          } else if (touch == 2
	        /* Covered */
	        ) {
	        ...
	        if (newChild != child) {
	          if (newChildren == null) newChildren = this.children.slice(0, i); // If the node's content was completely deleted by mapping,
	          ...
	          if (newChild.size == 0 && (newChild.length == 0 || newChildren.length || i == this.children.length)) {
	            if (newChild.length > 0 && i > 0) {
	              var last = newChildren.length - 1,
	                  lastChild = newChildren[last];
	      ...
	      var set = newLength == this.length && newChildren == null && newLocal == null ? this : new RangeSet(newLength, newSize + (newLocal || this.local).length, newLocal || this.local, newChildren || this.children);
	      return {
	        set: set,
	        escaped: escaped
	      };
	    ...
	    RangeSet.prototype.forEach = function (f) {
	      this.forEachInner(f, 0);
	    };
	      ...
	      return {
	        next: function () {
	          for (;;) {
	            ...
	            if (range.from > to) return; // Put the rest of the set back onto the heap

	            if (next.index < next.ranges.length) addToHeap(heap, next);else if (next.next) addIterToHeap(heap, next.next, 0);
	    ...
	    RangeSet.prototype.compare = function (other, textDiff, comparator, oldLen) {
	      ...
	      for (var _i = 0, textDiff_1 = textDiff; _i < textDiff_1.length; _i++) {
	        ...
	        if (range.fromB > newPos && (this != other || oldPos != newPos)) new RangeSetComparison(this, oldPos, other, newPos, range.fromB, comparator).run();
	        oldPos = range.toA;
	        newPos = range.toB;
	      }

	      if (oldPos < this.length || newPos < other.length || textDiff.length == 0) new RangeSetComparison(this, oldPos, other, newPos, newPos + (oldLen - oldPos), comparator).run();
	    };
	      ...
	      while (heap.length > 0) {
	        ...
	        if (next instanceof LocalSet) {
	          var range = next.ranges[next.index],
	              rFrom = range.from + next.offset,
	              rTo = range.to + next.offset;
	          if (rFrom > to) break; // Put the rest of the set back onto the heap

	          if (++next.index < next.ranges.length) addToHeap(heap, next);else if (next.next) addIterToHeap(heap, next.next, pos);
	  ...
	  exports.RangeSet = RangeSet; // Stack element for iterating over a range set
	  ...
	  function () {
	    function IteratedSet(offset, set) {
	      this.offset = offset;
	      this.set = set; // Index == -1 means the set's locals have not been yielded yet.
	  ...
	  function () {
	    ...
	    Object.defineProperty(LocalSet.prototype, "heapSide", {
	      get: function () {
	        return this.ranges[this.index].value.startSide;
	      },
	      enumerable: true,
	      configurable: true
	    });
	  ...
	  function iterRangeSet(stack, skipTo) {
	    ...
	    for (;;) {
	      ...
	      if (top_1.index == top_1.set.children.length) {
	        ...
	        var next = top_1.set.children[top_1.index],
	            start = top_1.offset;
	        top_1.index++;
	  ...
	  function addToHeap(heap, elt) {
	    ...
	    while (index > 0) {
	      var parentIndex = index >> 1,
	          parent_1 = heap[parentIndex];
	      if (compareHeapable(elt, parent_1) >= 0) break;
	      heap[index] = parent_1;
	  ...
	  function collapseSet(children, local, add, start, offset, length) {
	    var mustSort = local.length > 0 && add.length > 0,
	        off = 0;
	  ...
	  function appendRanges(local, children, ranges, start, offset, length, pos, childSize) {
	    ...
	    for (var i = start; i < ranges.length;) {
	      ...
	      for (; i < end; i++) {
	        var range = ranges[i];
	        if (range.to > endPos) insertSorted(local, range.move(-offset));else add.push(range);
	      ...
	      if (add.length) {
	        if (add.length == ranges.length) children.push(new RangeSet(endPos - pos, add.length, add.map(function (r) {
	          return r.move(-pos);
	        }), none));else children.push(RangeSet.empty.updateInner(add, null, 0, 0, pos, endPos - pos));
	        pos = endPos;
	      }
	    }
	  ...
	  function rebalanceChildren(local, children, childSize) {
	    var _loop_1 = function (i, off) {
	      ...
	      } else if (child.children.length == 0 && i < children.length - 1 && (next = children[i + 1]).size + child.size <= BASE_NODE_SIZE && next.children.length == 0) {
	        // Join two small leaf nodes
	        children.splice(i, 2, new RangeSet(child.length + next.length, child.size + next.size, child.local.concat(next.local.map(function (d) {
	          return d.move(child.length);
	        })), none));
	      } else {
	        // Join a number of nodes into a wrapper node
	        var joinTo = i + 1,
	            size = child.size,
	            length_1 = child.length;

	        if (child.size < childSize >> 1) {
	          for (; joinTo < children.length; joinTo++) {
	            var next_1 = children[joinTo],
	                totalSize = size + next_1.size;
	  ...
	  function () {
	    ...
	    ComparisonSide.prototype.findActive = function (to, value) {
	      for (var i = 0; i < this.active.length; i++) if (this.activeTo[i] == to && (this.active[i] == value || this.active[i].eq(value))) return i;
	  ...
	  function () {
	    ...
	    RangeSetComparison.prototype.forwardIter = function (side) {
	      for (; side > 0;) {
	        ...
	        } else if (nextA && (!nextB || nextA.offset < nextB.offset || nextA.offset == nextB.offset && (this.a.stack.length == 1 || nextA.set.length >= nextB.set.length))) {
	    ...
	    RangeSetComparison.prototype.run = function () {
	      for (;;) {
	        var nextA = this.a.nextPos,
	            nextB = this.b.nextPos;
	        if (nextA == FAR && nextB == FAR) break;
	        var diff = nextA - nextB || this.a.nextSide - this.a.nextSide;
	        if (diff < 0) this.advance(this.a, this.b);else this.advance(this.b, this.a);
	      }
	    };
	    ...
	    RangeSetComparison.prototype.advance = function (side, other) {
	      if (side.pointTo > -1) {
	        ...
	        if (!other.point || !side.point.eq(other.point)) this.comparator.comparePoint(this.pos, end, side.point, other.point);
	        this.pos = end;
	        if (end == this.end || other.pointTo == end && other.point.endSide == side.point.endSide) other.clearPoint();
	        side.clearPoint();
	        return;
	      }
	      ...
	      if (next instanceof LocalSet) {
	        ...
	        // opened.
	        var range = next.ranges[next.index++]; // The actual positions are offset relative to the node

	        var from = range.from + next.offset,
	            to = range.to + next.offset;
	        ...
	        if (to < other.pointTo || to == other.pointTo && range.value.startSide < other.point.endSide) return; // Otherwise, if the other side isn't a point, advance
	        ...
	        if (range.value.point) {
	          ...
	          to = Math.min(to, this.end); // Add this to the set of active ranges
	    ...
	    RangeSetComparison.prototype.advancePos = function (pos) {
	      ...
	      if (!sameSet(this.a.active, this.b.active)) this.comparator.compareRange(this.pos, pos, this.a.active, this.b.active);
	      this.pos = pos;
	    };
	  ...
	  function sameSet(a, b) {
	    ...
	    outer: for (var i = 0; i < a.length; i++) {
	      for (var j = 0; j < b.length; j++) if (a[i].eq(b[j])) continue outer;
	  ...
	  function touchesChanges(from, to, changes) {
	    ...
	    for (var _i = 0, changes_1 = changes; _i < changes_1.length; _i++) {
	      ...
	      if (change.to >= from && change.from <= to) {
	        if (change.from < from && change.to > to) result = 2
	        /* Covered */
	        ;else if (result == 1
	        /* No */
	        ) result = 0
	...
	var decoration = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    WidgetType.prototype.eq = function (value) {
	      return this.value === value;
	    };

	    WidgetType.prototype.updateDOM = function (dom) {
	      return false;
	    };
	    /** @internal */
	    ...
	    WidgetType.prototype.ignoreEvent = function (event) {
	      return true;
	    };
	  ...
	  var INLINE_BIG_SIDE = 1e8,
	      BLOCK_BIG_SIDE = 2e8;

	  var Decoration =
	  /** @class */
	  function (_super) {
	    __extends(Decoration, _super); // @internal
	    ...
	    Decoration.replace = function (from, to, spec) {
	      ...
	      if (from > to || from == to && startSide > 0 && endSide < 0) throw new RangeError("Invalid range for replacement decoration");
	    ...
	    Decoration.prototype.hasHeight = function () {
	    ...
	    Decoration.prototype.mapSimple = function (mapping, from, to) {
	      ...
	      if (from == to && this.startSide == this.endSide) return newFrom < 0 ? null : new rangeset.Range(newFrom, newFrom, this);
	  ...
	  function (_super) {
	    ...
	    MarkDecoration.prototype.eq = function (other) {
	      return this == other || other instanceof MarkDecoration && this.spec.tagName == other.spec.tagName && this.spec.class == other.spec.class && attributes.attrsEq(this.spec.attributes || null, other.spec.attributes || null);
	    };
	  ...
	  function (_super) {
	    ...
	    Object.defineProperty(PointDecoration.prototype, "type", {
	      ...
	      get: function () {
	        return this.startSide < this.endSide ? 3
	        /* WidgetRange */
	        : this.startSide < 0 ? 1
	        /* WidgetBefore */
	        : 2
	    ...
	    PointDecoration.prototype.map = function (mapping, from, to) {
	      ...
	      if (this.block) {
	        ...
	        var newFrom = type == 2
	        /* WidgetAfter */
	        ? mapping.mapPos(from, 1, src$1.MapMode.TrackAfter) : mapping.mapPos(from, -1, src$1.MapMode.TrackBefore);
	        var newTo = type == 3
	        /* WidgetRange */
	        ? mapping.mapPos(to, 1, src$1.MapMode.TrackAfter) : newFrom;
	  ...
	  function addRange(from, to, ranges) {
	    if (ranges[ranges.length - 1] + MIN_RANGE_GAP > from) ranges[ranges.length - 1] = to;else ranges.push(from, to);
	  }
	  ...
	  function joinRanges(a, b) {
	    ...
	    for (var iA = 0, iB = 0;;) {
	      if (iA < a.length && (iB == b.length || a[iA] < b[iB])) addRange(a[iA++], a[iA++], result);else if (iB < b.length) addRange(b[iB++], b[iB++], result);else break;
	    }
	  ...
	  function () {
	    ...
	    DecorationComparator.prototype.comparePoint = function (from, to, byA, byB) {
	      addRange(from, to, this.changes.content);
	      if (from > to || byA.heightRelevant || byB && byB.heightRelevant) addRange(from, to, this.changes.height);
	    };
	  ...
	  function () {
	    ...
	    HeightDecoScanner.prototype.point = function (from, to, value) {
	      addRange(from, to, this.ranges);
	    };

	    HeightDecoScanner.prototype.ignore = function (from, to, value) {
	      return from == to && !value.heightRelevant;
	    };
	...
	var buildview = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    ContentBuilder.prototype.posCovered = function () {
	      if (this.content.length == 0) return !this.breakAtStart && this.doc.lineAt(this.pos).start != this.pos;
	      var last = this.content[this.content.length - 1];
	      return !last.breakAfter && !(last instanceof blockview.BlockWidgetView && last.type == 1
	      /* WidgetBefore */
	      );
	    };

	    ContentBuilder.prototype.getLine = function () {
	      if (!this.curLine) this.content.push(this.curLine = new blockview.LineView());
	      return this.curLine;
	    };
	    ...
	    ContentBuilder.prototype.span = function (from, to, active) {
	      ...
	      for (var _i = 0, _a = active; _i < _a.length; _i++) {
	        ...
	        if (spec.attributes) for (var name_1 in spec.attributes) {
	          ...
	          if (name_1 == "class") {
	            ...
	            if (!attrs) attrs = {};
	            if (name_1 == "style" && attrs.style) value = attrs.style + ";" + value;
	            attrs[name_1] = value;
	          }
	        }
	      }
	    ...
	    ContentBuilder.prototype.point = function (from, to, deco, openStart, openEnd) {
	      ...
	      } else if (this.doc.lineAt(this.pos).start == this.pos) {
	        // Line decoration
	    ...
	    ContentBuilder.prototype.ignore = function () {
	      return false;
	    };
	  ...
	  function (_super) {
	    ...
	    NullWidget.prototype.toDOM = function () {
	      return document.createElement(this.value);
	    };

	    NullWidget.prototype.updateDOM = function (elt) {
	...
	var viewport = createCommonjsModule(function (module, exports) {
	  ...
	  function visiblePixelRange(dom, paddingTop) {
	    ...
	    var top = Math.max(0, Math.min(innerHeight, rect.top)),
	        bottom = Math.max(0, Math.min(innerHeight, rect.bottom));

	    for (var parent_1 = dom.parentNode; parent_1;) {
	  ...
	  function () {
	    ...
	    ViewportState.prototype.updateFromDOM = function (dom, paddingTop) {
	      var _a = visiblePixelRange(dom, paddingTop),
	          top = _a.top,
	          bottom = _a.bottom;

	      var dTop = top - this.top,
	    ...
	    ViewportState.prototype.getViewport = function (doc, heightMap, bias, scrollTo) {
	      ...
	      var viewport = new Viewport(heightMap.lineAt(this.top - marginTop * VIEWPORT_MARGIN, 1
	      /* ByHeight */
	      , doc, 0, 0).from, heightMap.lineAt(this.bottom + (1 - marginTop) * VIEWPORT_MARGIN, 1
	      /* ByHeight */
	      , doc, 0, 0).to); // If scrollTo is > -1, make sure the viewport includes that position

	      if (scrollTo > -1) {
	        if (scrollTo < viewport.from) {
	          var top_1 = heightMap.lineAt(scrollTo, 0
	          /* ByPos */
	          , doc, 0, 0).top;
	          viewport = new Viewport(heightMap.lineAt(top_1 - VIEWPORT_MARGIN / 2, 1
	          /* ByHeight */
	          , doc, 0, 0).from, heightMap.lineAt(top_1 + (this.bottom - this.top) + VIEWPORT_MARGIN / 2, 1
	          /* ByHeight */
	          , doc, 0, 0).to);
	        } else if (scrollTo > viewport.to) {
	          var bottom = heightMap.lineAt(scrollTo, 0
	          /* ByPos */
	          , doc, 0, 0).bottom;
	          viewport = new Viewport(heightMap.lineAt(bottom - (this.bottom - this.top) - VIEWPORT_MARGIN / 2, 1
	          /* ByHeight */
	          , doc, 0, 0).from, heightMap.lineAt(bottom + VIEWPORT_MARGIN / 2, 1
	          /* ByHeight */
	          , doc, 0, 0).to);
	        }
	      }
	    ...
	    ViewportState.prototype.coveredBy = function (doc, viewport, heightMap, bias) {
	      ...
	      var top = heightMap.lineAt(viewport.from, 0
	      /* ByPos */
	      , doc, 0, 0).top;
	      var bottom = heightMap.lineAt(viewport.to, 0
	      /* ByPos */
	      , doc, 0, 0).bottom;
	      return (viewport.from == 0 || top <= this.top - Math.max(MIN_COVER_MARGIN, Math.min(-bias, MAX_COVER_MARGIN))) && (viewport.to == doc.length || bottom >= this.bottom + Math.max(MIN_COVER_MARGIN, Math.min(bias, MAX_COVER_MARGIN)));
	    };
	  ...
	  function () {
	    ...
	    Viewport.prototype.clip = function (pos) {
	      return Math.max(this.from, Math.min(this.to, pos));
	    };

	    Viewport.prototype.eq = function (b) {
	  ...
	  function () {
	    function DOMObserver(docView, onChange, onScrollChanged) {
	      ...
	      this.observer = new MutationObserver(function (mutations) {
	        return _this.flush(mutations);
	      });
	      if (useCharData) this.onCharData = function (event) {
	        _this.charDataQueue.push({
	          target: event.target,
	          type: "characterData",
	          oldValue: event.prevValue
	        });

	        if (_this.charDataTimeout == null) _this.charDataTimeout = setTimeout(function () {
	          return _this.flush();
	        }, 20);
	      };

	      this.onSelectionChange = function () {
	        if (_this.docView.root.activeElement == _this.dom) _this.flush();
	      };
	    ...
	    DOMObserver.prototype.flush = function (records) {
	      ...
	      if (this.charDataQueue.length) records = records.concat(this.takeCharRecords());
	      ...
	      if (!apply || !this.onChange(from, to, typeOver)) {
	        if (this.docView.dirty) this.ignore(function () {
	          return _this.docView.sync();
	        });
	...
	var heightmap = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    HeightOracle.prototype.setDoc = function (doc) {
	      this.doc = doc;
	      return this;
	    };
	  ...
	  exports.HeightOracle = HeightOracle; // This object is used by `updateHeight` to make DOM measurements
	  ...
	  function () {
	    ...
	    HeightMap.prototype.decomposeLeft = function (to, result) {
	      result.push(this);
	    };

	    HeightMap.prototype.decomposeRight = function (from, result) {
	      result.push(this);
	    };
	    ...
	    HeightMap.prototype.applyChanges = function (decorations, oldDoc, oracle, changes) {
	      ...
	      for (var i = changes.length - 1; i >= 0; i--) {
	        var _a = changes[i],
	            ...
	            fromB = _a.fromB,
	            toB = _a.toB;
	        var start = me.lineAt(fromA, 2
	        /* ByPosNoHeight */
	        , oldDoc, 0, 0);
	        var end = start.to >= toA ? start : me.lineAt(toA, 2
	        /* ByPosNoHeight */
	        , oldDoc, 0, 0);
	        toB += end.to - toA;
	        toA = end.to;
	        ...
	        while (i > 0 && start.from <= changes[i - 1].toA) {
	          ...
	          i--;
	          if (fromA < start.from) start = me.lineAt(fromA, 2
	          /* ByPosNoHeight */
	          , oldDoc, 0, 0);
	        }
	  ...
	  function (_super) {
	    ...
	    HeightMapBlock.prototype.toString = function () {
	  ...
	  function (_super) {
	    ...
	    function HeightMapText(length, height) {
	      var _this = _super.call(this, length, height, 0
	      /* Text */
	      ) || this;
	  ...
	  function (_super) {
	    ...
	    HeightMapGap.prototype.blockAt = function (height, doc, top, offset) {
	      var _a = this.lines(doc, offset),
	          firstLine = _a.firstLine,
	          lastLine = _a.lastLine,
	          lineHeight = _a.lineHeight;

	      var line = Math.max(0, Math.min(lastLine - firstLine, Math.floor((height - top) / lineHeight)));

	      var _b = doc.line(firstLine + line),
	          start = _b.start,
	          length = _b.length;

	      return new BlockInfo(start, length, top + lineHeight * line, lineHeight, 0
	      /* Text */
	      );
	    };
	    ...
	    HeightMapGap.prototype.lineAt = function (value, type, doc, top, offset) {
	      ...
	      /* ByPosNoHeight */
	          ...
	          var _a = doc.lineAt(value),
	              start_1 = _a.start,
	              end = _a.end;

	          return new BlockInfo(start_1, end - start_1, 0, 0, 0
	          /* Text */
	          );
	        }

	      var _b = this.lines(doc, offset),
	          firstLine = _b.firstLine,
	          lineHeight = _b.lineHeight;

	      var _c = doc.lineAt(value),
	          start = _c.start,
	          length = _c.length,
	          number = _c.number;

	      return new BlockInfo(start, length, top + lineHeight * (number - firstLine), lineHeight, 0
	      /* Text */
	      );
	    };
	    ...
	    HeightMapGap.prototype.forEachLine = function (from, to, doc, top, offset, f) {
	      ...
	      for (var line = firstLine; line <= lastLine; line++) {
	        var _b = doc.line(line),
	            start = _b.start,
	            end = _b.end;

	        if (start > to) break;
	        if (end >= from) f(new BlockInfo(start, end - start, top, top += lineHeight, 0
	        /* Text */
	        ));
	      }
	    };
	    ...
	    HeightMapGap.prototype.toString = function () {
	  ...
	  function (_super) {
	    ...
	    function HeightMapBranch(left, brk, right) {
	      var _this = _super.call(this, left.length + brk + right.length, left.height + right.height, brk | (left.outdated || right.outdated ? 2
	      /* Outdated */
	      : 0)) || this;
	    ...
	    HeightMapBranch.prototype.lineAt = function (value, type, doc, top, offset) {
	      ...
	      var left = type == 1
	      /* ByHeight */
	      ? value < rightTop || this.right.height == 0 : value < rightOffset;
	    ...
	    HeightMapBranch.prototype.forEachLine = function (from, to, doc, top, offset, f) {
	      ...
	      if (this.break) {
	        ...
	        var mid = this.lineAt(rightOffset, 0
	        /* ByPos */
	        , doc, top, offset);
	    ...
	    HeightMapBranch.prototype.toString = function () {
	  ...
	  function () {
	    ...
	    NodeBuilder.prototype.point = function (from, to, deco) {
	      ...
	      if (this.lineEnd > -1 && this.lineEnd < this.pos) this.lineEnd = this.oracle.doc.lineAt(this.pos).end;
	    };

	    NodeBuilder.prototype.enterLine = function () {
	      if (this.lineStart > -1) return;

	      var _a = this.oracle.doc.lineAt(this.pos),
	          start = _a.start,
	          end = _a.end;
	      ...
	      if (this.writtenTo < start) {
	        if (this.writtenTo < start - 1 || this.nodes[this.nodes.length - 1] == null) this.nodes.push(new HeightMapGap(start - this.writtenTo - 1));
	        this.nodes.push(null);
	      }

	      if (this.pos > start) this.nodes.push(new HeightMapText(this.pos - start, -1));
	    ...
	    NodeBuilder.prototype.addBlock = function (block) {
	      ...
	      if (block.type == 2
	      /* WidgetAfter */
	      && !this.isCovered) this.ensureLine();
	    ...
	    NodeBuilder.prototype.finish = function (from) {
	      ...
	      if (this.lineStart > -1 && !(last instanceof HeightMapText) && !this.isCovered) this.nodes.push(new HeightMapText(0, -1));else if (this.writtenTo < this.pos || last == null) this.nodes.push(new HeightMapGap(this.pos - this.writtenTo));
	      var pos = from;
	    ...
	    NodeBuilder.prototype.ignore = function (from, to, value) {
	      return from == to && !value.heightRelevant;
	...
	var extension$1 = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    ViewField.decorations = function (_a) {
	      ...
	      return new ViewField({
	        ...
	        effects: [ViewField.decorationEffect(function (d) {
	          return d;
	        })]
	      }).extension;
	    };
	  ...
	  function attributeField(effect) {
	    return function (value, update) {
	      return new ViewField({
	        create: value instanceof Function ? value : function () {
	          return value;
	        },
	        update: update || function (a) {
	          return a;
	        },
	        effects: [effect(function (a) {
	          return a;
	        })]
	      }).extension;
	    };
	  ...
	  /** @class */
	  function () {
	    function ViewUpdate(view, transactions, // @internal
	    metadata) {
	      ...
	      this.changes = transactions.reduce(function (chs, tr) {
	        return chs.appendSet(tr.changes);
	      }, src$1.ChangeSet.empty);
	  ...
	  function getField(field, fields, values, defaultValue) {
	    ...
	    if (index < 0) {
	      if (defaultValue === undefined) throw new RangeError("Field isn't present");else return defaultValue;
	    }
	...
	var docview = createCommonjsModule(function (module, exports) {
	  ...
	  function (_super) {
	    ...
	    function DocView(view, onDOMChange) {
	      ...
	      _this.layoutCheckScheduled = -1; // A document position that has to be scrolled into view at the next layout check

	      _this.scrollIntoView = -1;
	      _this.paddingTop = 0;
	      _this.paddingBottom = 0; // Track a minimum width for the editor. When measuring sizes in
	      ...
	      _this.minWidthTo = 0; // Track whether the DOM selection was set in a lossy way, so that
	      ...
	      _this.observer = new domobserver.DOMObserver(_this, onDOMChange, function () {
	        return _this.checkLayout();
	      });
	      return _this;
	    }
	    ...
	    DocView.prototype.init = function (state) {
	      ...
	      this.layoutCheckScheduled = requestAnimationFrame(function () {
	        return _this.checkLayout();
	      });
	    ...
	    DocView.prototype.update = function (update, scrollIntoView) {
	      ...
	      var contentChanges = this.computeUpdate(state, update, false, changedRanges, 0, scrollIntoView); // When the DOM nodes around the selection are moved to another
	      // parent, Chrome sometimes reports a different selection through
	      // getSelection than the one that it actually shows to the user.
	      // This forces a selection update when lines are joined to work
	      // around that. Issue #54

	      if (browser.default.chrome && !this.compositionDeco.size && update && update.changes.changes.some(function (ch) {
	        return ch.text.length > 1;
	      })) this.forceSelectionUpdate = true;

	      if (this.dirty == 0
	      /* Not */
	      && contentChanges.length == 0 && this.state.selection.primary.from >= this.viewport.from && this.state.selection.primary.to <= this.viewport.to && (!update || update.metadata.length == 0)) {
	    ...
	    DocView.prototype.updateInner = function (changes, oldLength) {
	      ...
	      var visible = this.viewport,
	          viewports = [visible];
	      var _a = this.state.selection.primary,
	          head = _a.head,
	          anchor = _a.anchor;

	      if (head < visible.from || head > visible.to) {
	        var _b = this.lineAt(head, 0),
	            from = _b.from,
	            to = _b.to;
	    ...
	    DocView.prototype.updateChildren = function (changes, viewports, oldLength) {
	      ...
	      for (var i = changes.length - 1;; i--) {
	        ...
	        var _a = buildview.ContentBuilder.build(this.state.doc, fromB, toB, allDeco),
	            content = _a.content,
	            breakAtStart = _a.breakAtStart;

	        var _b = cursor.findPos(toA, 1),
	            toI = _b.i,
	            toOff = _b.off;

	        var _c = cursor.findPos(fromA, -1),
	            fromI = _c.i,
	            fromOff = _c.off;
	    ...
	    DocView.prototype.replaceRange = function (fromI, fromOff, toI, toOff, content, breakAtStart) {
	      ...
	      if (fromI == toI && !breakAtStart && !breakAtEnd && content.length < 2 && before.merge(fromOff, toOff, content.length ? last : null, fromOff == 0)) return;
	    ...
	    DocView.prototype.updateSelection = function (takeFocus) {
	      ...
	      var primary = this.state.selection.primary; // FIXME need to handle the case where the selection falls inside a block range

	      var anchor = this.domAtPos(primary.anchor);
	      var head = this.domAtPos(primary.head);
	      var domSel = this.root.getSelection(); // If the selection is already here, or in an equivalent position, don't touch it

	      if (this.forceSelectionUpdate || !dom.isEquivalentPosition(anchor.node, anchor.offset, domSel.anchorNode, domSel.anchorOffset) || !dom.isEquivalentPosition(head.node, head.offset, domSel.focusNode, domSel.focusOffset)) {
	        this.forceSelectionUpdate = false;
	        this.observer.ignore(function () {
	          var _a; // Selection.extend can be used to create an 'inverted' selection
	          ...
	          if (domSel.extend) {
	            ...
	            var range = document.createRange();
	            if (primary.anchor > primary.head) _a = [head, anchor], anchor = _a[0], head = _a[1];
	    ...
	    DocView.prototype.lineAt = function (pos, editorTop) {
	      ...
	      return this.heightMap.lineAt(pos, 0
	      /* ByPos */
	      , this.state.doc, editorTop + this.paddingTop, 0);
	    };

	    DocView.prototype.lineAtHeight = function (height, editorTop) {
	      if (editorTop == null) editorTop = this.dom.getBoundingClientRect().top;
	      return this.heightMap.lineAt(height, 1
	      /* ByHeight */
	      , this.state.doc, editorTop + this.paddingTop, 0);
	    };
	    ...
	    DocView.prototype.computeUpdate = function (state, update, initializing, contentChanges, viewportBias, scrollIntoView) {
	      for (var i = 0;; i++) {
	        ...
	        var viewportChange = this.viewport ? !viewport.eq(this.viewport) : true; // When the viewport is stable and no more iterations are needed, return

	        if (!viewportChange && !update && !initializing) return contentChanges; // After 5 tries, or 
	        ...
	        if (initializing) this.view.initInner(state, viewport);else this.view.updateInner(update || new extension$1.ViewUpdate(this.view), viewport); // For the composition decoration, use none on init, recompute
	        // when handling transactions, and use the previous value
	        // otherwise.

	        if (!this.view.inputState.composing) this.compositionDeco = decoration.Decoration.none;else if (update && update.transactions.length) this.compositionDeco = computeCompositionDeco(this.view, contentChanges);
	        var decorations = this.view.getEffect(extension$1.ViewField.decorationEffect).concat(this.compositionDeco); // If the decorations are stable, stop.

	        if (!update && !initializing && sameArray(decorations, this.decorations)) return contentChanges; // Compare the decorations (between document changes)

	        var _a = decoChanges(update ? contentChanges : none, decorations, this.decorations, prevState.doc.length),
	            content = _a.content,
	            height = _a.height;

	        this.decorations = decorations; // Update the heightmap with these changes. If this is the first
	        // iteration and the document changed, also include decorations
	        // for inserted ranges.

	        var heightChanges = extendWithRanges(none, height);
	        if (update) heightChanges = extendWithRanges(heightChanges, decoration.heightRelevantDecorations(decorations, contentChanges));
	        this.heightMap = this.heightMap.applyChanges(decorations, this.state.doc, this.heightOracle, heightChanges); // Accumulate content changes so that they can be redrawn

	        contentChanges = extendWithRanges(contentChanges, content); // Make sure only one iteration is marked as required / state changing
	    ...
	    DocView.prototype.checkLayout = function (forceFull) {
	      ...
	      var scrollBias = 0;
	      if (forceFull) this.viewportState.coverEverything();else scrollBias = this.viewportState.updateFromDOM(this.dom, this.paddingTop);
	      if (this.viewportState.top >= this.viewportState.bottom) return; // We're invisible!

	      var lineHeights = this.measureVisibleLineHeights(),
	          refresh = false;

	      if (this.heightOracle.mustRefresh(lineHeights)) {
	        var _a = this.measureTextSize(),
	            lineHeight = _a.lineHeight,
	            charWidth = _a.charWidth;
	      ...
	      this.view.withUpdating(function () {
	        ...
	        for (var i = 0;; i++) {
	          ...
	          if (covered && !_this.heightOracle.heightChanged) break;
	          if (!update) update = new extension$1.ViewUpdate(_this.view);
	    ...
	    DocView.prototype.scrollPosIntoView = function (pos) {
	      ...
	      if (rect) dom.scrollRectIntoView(this.dom, rect);
	    };
	    ...
	    DocView.prototype.domAtPos = function (pos) {
	      var _a = this.childCursor().findPos(pos, -1),
	          i = _a.i,
	          off = _a.off;

	      for (; i < this.children.length - 1;) {
	        var child = this.children[i];
	        if (off < child.length || child instanceof blockview.LineView) break;
	        i++;
	        off = 0;
	      }
	    ...
	    DocView.prototype.setSelectionDirty = function () {
	      ...
	      if (this.selectionDirty == null) this.selectionDirty = requestAnimationFrame(function () {
	    ...
	    DocView.prototype.computeGapDeco = function (viewports, docLength) {
	      ...
	      for (var pos = 0, i = 0;; i++) {
	        ...
	        if (end > pos) {
	          ...
	          deco.push(decoration.Decoration.replace(pos, end, {
	            widget: new GapWidget(height),
	            block: true,
	            inclusive: true
	          }));
	        }
	  ...
	  exports.DocView = DocView; // Browsers appear to reserve a fixed amount of bits for height
	  ...
	  function decoChanges(diff, decorations, oldDecorations, oldLength) {
	    ...
	    for (var i = decorations.length - 1; i >= 0; i--) {
	      var deco = decorations[i],
	          oldDeco = i < oldDecorations.length ? oldDecorations[i] : decoration.Decoration.none;
	    ...
	    for (var dI = 0, rI = 0, posA = 0, posB = 0;; dI++) {
	      ...
	      while (rI < ranges.length && ranges[rI] < end) {
	        ...
	        var fromB = Math.max(posB, from),
	            toB = Math.min(end, to);
	        if (fromB <= toB) new src$1.ChangedRange(fromB + off, toB + off, fromB, toB).addToSet(result);
	  ...
	  function computeCompositionDeco(view, changes) {
	    ...
	    if (!textNode) return decoration.Decoration.none;
	    ...
	    var newFrom = src$1.ChangedRange.mapPos(from, 1, changes),
	        newTo = Math.max(newFrom, src$1.ChangedRange.mapPos(to, -1, changes));
	    var text = textNode.nodeValue,
	        doc = view.state.doc;

	    if (newTo - newFrom < text.length) {
	      if (doc.slice(newFrom, Math.min(doc.length, newFrom + text.length)) == text) newTo = newFrom + text.length;else if (doc.slice(Math.max(0, newTo - text.length), newTo) == text) newFrom = newTo - text.length;else return decoration.Decoration.none;
	  ...
	  function (_super) {
	    ...
	    CompositionWidget.prototype.eq = function (value) {
	      return this.value.top == value.top && this.value.text == value.text;
	    };

	    CompositionWidget.prototype.toDOM = function () {
	      return this.value.top;
	    };

	    CompositionWidget.prototype.ignoreEvent = function () {
	      return false;
	    };

	    Object.defineProperty(CompositionWidget.prototype, "customView", {
	      get: function () {
	        return inlineview.CompositionView;
	      },
	      enumerable: true,
	      configurable: true
	    });
	...
	var cursor = createCommonjsModule(function (module, exports) {
	  ...
	  function movePos(view, start, direction, granularity, action) {
	    ...
	    var dir = direction == "forward" || direction == "right" ? 1 : -1; // Can only query native behavior when Selection.modify is
	    ...
	    if (sel.modify && context && !context.nearViewportEnd(view) && view.hasFocus() && granularity != "word" && !(granularity == "line" && (browser.default.gecko || view.state.selection.ranges.length > 1))) {
	      return view.docView.observer.ignore(function () {
	        ...
	        var equiv = (!browser.default.chrome || prepared.lines.length == 0) && dom.isEquivalentPosition(startDOM.node, startDOM.offset, sel.focusNode, sel.focusOffset) && false; // Firefox skips an extra character ahead when extending across
	        // an uneditable element (but not when moving)

	        if (prepared.atWidget && browser.default.gecko && action == "extend") action = "move";
	        if (action == "move" && !(equiv && sel.isCollapsed)) sel.collapse(startDOM.node, startDOM.offset);else if (action == "extend" && !equiv) sel.extend(startDOM.node, startDOM.offset);
	  ...
	  function moveLineByColumn(doc, tabSize, pos, dir) {
	    var line = doc.lineAt(pos); // FIXME also needs goal column?

	    var col = 0;

	    for (var iter = doc.iterRange(line.start, pos); !iter.next().done;) col = src.countColumn(iter.value, col, tabSize);

	    if (dir < 0 && line.start == 0) return 0;else if (dir > 0 && line.end == doc.length) return line.end;
	    ...
	    for (var iter = doc.iterRange(otherLine.start, otherLine.end); seen >= col && !iter.next().done;) {
	      var _a = src.findColumn(iter.value, seen, col, tabSize),
	          offset = _a.offset,
	          leftOver = _a.leftOver;
	    ...
	    if (context == null) {
	      for (var pos = start;; pos += dir) {
	        ...
	        if (!src.isExtendingChar(dir < 0 ? doc.slice(pos - 1, pos) : doc.slice(pos, pos + 1))) {
	          if (dir < 0) return pos - 1;else if (pos != start) return pos;
	        }
	      }
	    }
	  ...
	  function moveWord(view, start, direction) {
	    ...
	    for (var pos = start, i = 0;; i++) {
	      ...
	      var away = pos < group.from && pos > group.to; // If the group is away from its start position, we jumped over a
	  ...
	  function getGoalColumn(view, pos, column) {
	    for (var _i = 0, _a = view.inputState.goalColumns; _i < _a.length; _i++) {
	      var goal_1 = _a[_i];
	      if (goal_1.pos == pos) return goal_1;
	    }
	  ...
	  function () {
	    ...
	    LineContext.get = function (view, pos) {
	      for (var i = 0, off = 0;; i++) {
	        var line = view.docView.children[i],
	            end = off + line.length;

	        if (end >= pos) {
	          if (line instanceof blockview.LineView) return new LineContext(line, off, i);
	    ...
	    LineContext.prototype.nearViewportEnd = function (view, side) {
	      ...
	      for (var _i = 0, _a = view.docView.viewports; _i < _a.length; _i++) {
	        var _b = _a[_i],
	            from = _b.from,
	            to = _b.to;
	        if (from > 0 && from == this.start && side <= 0 || to < view.state.doc.length && to == this.start + this.line.length && side >= 0) return true;
	      }
	    ...
	    LineContext.prototype.prepareForQuery = function (view, pos) {
	      ...
	      function maybeHide(view) {
	        if (!(view instanceof inlineview.TextView)) atWidget = true;
	        if (view.length > 0) return false;
	        view.dom.remove();
	        if (linesToSync.indexOf(view.parent) < 0) linesToSync.push(view.parent);
	        return true;
	      }

	      var _a = this.line.childPos(pos - this.start),
	          i = _a.i,
	          off = _a.off;

	      if (off == 0) {
	        for (var j = i; j < this.line.children.length; j++) if (!maybeHide(this.line.children[j])) break;

	        for (var j = i; j > 0; j--) if (!maybeHide(this.line.children[j - 1])) break;
	      }
	      ...
	      function addForLine(line, omit) {
	        ...
	        for (var i_1 = 0, off_1 = 0; i_1 <= line.children.length; i_1++) {
	          ...
	          if ((!next || !(next instanceof inlineview.TextView)) && off_1 != omit && (i_1 == 0 || !(line.children[i_1 - 1] instanceof inlineview.TextView))) {
	            line.dom.insertBefore(document.createTextNode("\u200b"), next ? next.dom : null);
	            if (linesToSync.indexOf(line) < 0) linesToSync.push(line);
	          }

	          if (next) off_1 += next.length;
	        }
	      }

	      if (this.index > 0) addForLine(this.line.parent.children[this.index - 1]);
	      addForLine(this.line, pos - this.start);
	      if (this.index < this.line.parent.children.length - 1) addForLine(this.line.parent.children[this.index + 1]);
	      return {
	        lines: linesToSync,
	        atWidget: atWidget
	      };
	  ...
	  exports.LineContext = LineContext; // Search the DOM for the {node, offset} position closest to the given
	  ...
	  function domPosAtCoords(parent, x, y) {
	    ...
	    if (!closestX && closest.contentEditable == "true") domPosAtCoords(closest, clipX, y);
	...
	var input = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    InputState.prototype.runCustomHandlers = function (type, view, event) {
	      ...
	      if (handlers) for (var _i = 0, handlers_1 = handlers; _i < handlers_1.length; _i++) {
	  ...
	  function () {
	    function MouseSelection(inputState, view, event, update) {
	      ...
	      var _a = this.queryPos(event),
	          pos = _a.pos,
	          bias = _a.bias;

	      this.startPos = this.curPos = pos;
	      this.startBias = this.curBias = bias;
	      this.dragging = isInPrimarySelection(view, this.startPos, event) ? null : false; // When clicking outside of the selection, immediately apply the
	    ...
	    MouseSelection.prototype.queryPos = function (event) {
	      var pos = this.view.posAtCoords({
	        x: event.clientX,
	        y: event.clientY
	      });
	      var coords = this.view.coordsAtPos(pos);
	      var bias = !coords ? 1 : coords.top > event.clientY ? -1 : coords.bottom < event.clientY ? 1 : coords.left > event.clientX ? -1 : 1;
	      return {
	        pos: pos,
	        bias: bias
	      };
	    ...
	    MouseSelection.prototype.select = function () {
	      ...
	      if (!selection.eq(this.view.state.selection)) this.view.dispatch(this.view.state.t().setSelection(selection).addMeta(src$1.Transaction.userEvent("pointer")));
	    };
	  ...
	  function isInPrimarySelection(view, pos, event) {
	    ...
	    if (pos > primary.from && pos < primary.to) return true; // On boundary clicks, check whether the coordinates are inside the
	  ...
	  function eventBelongsToEditor(view, event) {
	    ...
	    for (var node = event.target; node != view.contentDOM; node = node.parentNode) if (!node || node.nodeType == 11 || node.cmView && node.cmView.ignoreEvent(event)) return false;
	  ...
	  function customHandlers(view) {
	    ...
	    for (var _i = 0, _a = view.behavior.get(extension$1.handleDOMEvents); _i < _a.length; _i++) {
	      ...
	      for (var eventType in handlers_2) (result[eventType] || (result[eventType] = [])).push(handlers_2[eventType]);
	    }
	  ...
	  var brokenClipboardAPI = browser.default.ie && browser.default.ie_version < 15 || browser.default.ios && browser.default.webkit_version < 604;
	  ...
	  function doPaste(view, text) {
	    view.dispatch(view.state.t().replaceSelection(text).addMeta(src$1.Transaction.userEvent("paste")).scrollIntoView());
	  }
	  ...
	  function mustCapture(event) {
	    ...
	    var code = event.keyCode,
	        macCtrl = browser.default.mac && mods == 1
	    /* Ctrl */
	    ;
	    return code == 8 || macCtrl && code == 72 || // Backspace, Ctrl-h on Mac
	    code == 46 || macCtrl && code == 68 || // Delete, Ctrl-d on Mac
	    code == 27 || // Esc
	    mods == (browser.default.mac ? 8
	    /* Meta */
	    : 1
	    /* Ctrl */
	    ) && ( // Ctrl/Cmd-[biyz]
	  ...
	  handlers.mousedown = function (view, event) {
	    if (event.button == 0) view.startMouseSelection(event, updateMouseSelection(event.detail));
	  };
	  ...
	  function rangeForClick(view, pos, bias, type) {
	    if (type == 1) {
	      ...
	      if (context) return new src$1.SelectionRange(context.start + context.line.length, context.start);

	      var _a = view.state.doc.lineAt(pos),
	          start = _a.start,
	          end = _a.end;
	  ...
	  function updateMouseSelection(type) {
	    return function (view, startSelection, startPos, startBias, curPos, curBias, extend, multiple) {
	      ...
	      if (extend) return startSelection.replaceRange(startSelection.primary.extend(range.from, range.to));else if (multiple) return startSelection.addRange(range);else return src$1.EditorSelection.create([range]);
	    };
	  }
	  ...
	  handlers.drop = function (view, event) {
	    ...
	    tr.change(change).setSelection(src$1.EditorSelection.single(dropPos, dropPos + change.length)).addMeta(src$1.Transaction.userEvent("drop"));
	    view.focus();
	    view.dispatch(tr);
	  };
	  ...
	  function forceClearComposition(view) {
	    if (view.docView.compositionDeco.size) view.update([], [compositionEndSlot(null)]);
	  }
	...
	var domchange = createCommonjsModule(function (module, exports) {
	  ...
	  function applyDOMChange(view, start, end, typeOver) {
	    var change, newSel;
	    var sel = view.state.selection.primary,
	        bounds;
	    ...
	    if (start > -1 && (bounds = view.docView.domBoundsAround(start, end, 0))) {
	      ...
	      if (diff) change = new src$1.Change(from + diff.from, from + diff.toA, reader.text.slice(diff.from, diff.toB).split(LINE_SEP));
	    } else if (view.hasFocus()) {
	      var domSel = view.root.getSelection();
	      var _a = view.docView,
	          iHead = _a.impreciseHead,
	          iAnchor = _a.impreciseAnchor;
	      var head = iHead && iHead.node == domSel.focusNode && iHead.offset == domSel.focusOffset ? view.state.selection.primary.head : view.docView.posFromDOM(domSel.focusNode, domSel.focusOffset);
	      var anchor = iAnchor && iAnchor.node == domSel.anchorNode && iAnchor.offset == domSel.anchorOffset ? view.state.selection.primary.anchor : dom.selectionCollapsed(domSel) ? head : view.docView.posFromDOM(domSel.anchorNode, domSel.anchorOffset);
	      if (head != sel.head || anchor != sel.anchor) newSel = src$1.EditorSelection.single(anchor, head);
	    }

	    if (!change && !newSel) return false; // Heuristic to notice typing over a selected character

	    if (!change && typeOver && !sel.empty && newSel && newSel.primary.empty) change = new src$1.Change(sel.from, sel.to, view.state.doc.sliceLines(sel.from, sel.to));
	    ...
	    if (change) {
	      ...
	      if (browser.default.android && (change.from == sel.from && change.to == sel.to && change.length == 1 && change.text.length == 2 && dispatchKey(view, "Enter", 10) || change.from == sel.from - 1 && change.to == sel.to && change.length == 0 && dispatchKey(view, "Backspace", 8) || change.from == sel.from && change.to == sel.to + 1 && change.length == 0 && dispatchKey(view, "Delete", 46))) return view.state != startState;
	      var tr = startState.t();
	      ...
	      if (change.from >= sel.from && change.to <= sel.to && change.to - change.from >= (sel.to - sel.from) / 3) {
	        ...
	        tr.change(change);
	        if (newSel && !tr.selection.primary.eq(newSel.primary)) tr.setSelection(tr.selection.replaceRange(newSel.primary));
	      }
	      ...
	      if (view.inputState.lastSelectionTime > Date.now() - 50) {
	        if (view.inputState.lastSelectionOrigin == "keyboard") tr.scrollIntoView();else tr.addMeta(src$1.Transaction.userEvent(view.inputState.lastSelectionOrigin));
	      }
	  ...
	  function () {
	    ...
	    DOMReader.prototype.readRange = function (start, end) {
	      ...
	      for (var cur = start;;) {
	        ...
	        if (next == end) break;
	        var view = cur.cmView;
	        if ((view ? view.breakAfter : isBlockNode(cur)) || isBlockNode(next) && cur.nodeName != "BR") this.text += LINE_SEP;
	        cur = next;
	      }
	    ...
	    DOMReader.prototype.readNode = function (node) {
	      ...
	      var text;
	      if (fromView != null) text = fromView.join(LINE_SEP);else if (node.nodeType == 3) text = node.nodeValue;else if (node.nodeName == "BR") text = node.nextSibling ? LINE_SEP : "";else if (node.nodeType == 1) this.readRange(node.firstChild, null);
	  ...
	  function selectionPoints(dom, root) {
	    ...
	    var _a = root.getSelection(),
	        anchorNode = _a.anchorNode,
	        anchorOffset = _a.anchorOffset,
	        focusNode = _a.focusNode,
	        focusOffset = _a.focusOffset;
	...
	var editorview = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    function EditorView(config) {
	      ...
	      this.contentAttrs = new AttrsFor(extension$1.ViewField.contentAttributeEffect, this.contentDOM, function () {
	        return {
	          spellcheck: "false",
	      ...
	      this.editorAttrs = new AttrsFor(extension$1.ViewField.editorAttributeEffect, this.dom, function (view) {
	        return {
	          class: "codemirror " + exports.styles.wrapper + (view.hasFocus() ? " codemirror-focused" : "")
	        };
	      });

	      this.dispatch = config.dispatch || function (tr) {
	        return _this.update([tr]);
	      };

	      this.root = config.root || document;
	      this.docView = new docview.DocView(this, function (start, end, typeOver) {
	        return domchange.applyDOMChange(_this, start, end, typeOver);
	      });
	    ...
	    EditorView.prototype.setState = function (state, extensions) {
	      ...
	      this.withUpdating(function () {
	        ...
	        if (_this.behavior.foreign.length) throw new Error("Non-ViewExtension extensions found when setting view state");
	        _this.inputState = new input.InputState(_this);

	        _this.docView.init(state);

	        _this.plugins = _this.behavior.get(extension$1.viewPlugin).map(function (spec) {
	          return spec(_this);
	        });
	    ...
	    EditorView.prototype.update = function (transactions, metadata) {
	      ...
	      for (var _i = 0, transactions_1 = transactions; _i < transactions_1.length; _i++) {
	        ...
	        if (tr.startState != state) throw new RangeError("Trying to update state with a transaction that doesn't start from the current state.");
	        state = tr.apply();
	      }

	      this.withUpdating(function () {
	        var update = transactions.length > 0 || metadata.length > 0 ? new extension$1.ViewUpdate(_this, transactions, metadata) : null;
	        if (state.doc != _this.state.doc || transactions.some(function (tr) {
	          return tr.selectionSet && !tr.getMeta(src$1.Transaction.preserveGoalColumn);
	        })) _this.inputState.goalColumns.length = 0;

	        _this.docView.update(update, transactions.some(function (tr) {
	          return tr.scrolledIntoView;
	        }) ? state.selection.primary.head : -1);
	    ...
	    EditorView.prototype.updateInner = function (update, viewport) {
	      ...
	      for (var i = 0; i < this.fields.length; i++) this.fieldValues.push(this.fields[i].update(update.prevFieldValues[i], update));
	    }; // @internal


	    EditorView.prototype.withUpdating = function (f) {
	      if (this.updating) throw new Error("Calls to EditorView.update or EditorView.setState are not allowed in extension update or create methods");
	      this.updating = true;
	    ...
	    EditorView.prototype.getEffect = function (type) {
	      ...
	      for (var i = 0; i < this.fieldValues.length; i++) {
	        ...
	        if (accessor) result.push(accessor(this.fieldValues[i]));
	      }
	    ...
	    Object.defineProperty(EditorView.prototype, "defaultCharacterWidth", {
	      get: function () {
	        return this.docView.heightOracle.charWidth;
	      },
	      enumerable: true,
	      configurable: true
	    });
	    Object.defineProperty(EditorView.prototype, "defaultLineHeight", {
	      get: function () {
	        return this.docView.heightOracle.lineHeight;
	      },
	      enumerable: true,
	      configurable: true
	    });
	  ...
	  function () {
	    ...
	    AttrsFor.prototype.update = function (view) {
	      ...
	      for (var _i = 0, _a = view.getEffect(this.effect); _i < _a.length; _i++) {
	        var spec = _a[_i];
	        if (spec) attrs = attributes.combineAttrs(spec, attrs);
	      }
	...
	var core = createCommonjsModule(function (module, exports) {
	  ...
	  function addChanges(branch, changes, inverted, selectionBefore, maxLen, mayMerge) {
	    if (branch.length) {
	      ...
	      if (lastItem.selection && lastItem.isChange == Boolean(inverted) && mayMerge(lastItem)) return inverted ? updateBranch(branch, branch.length - 1, maxLen, new Item(lastItem.map.appendSet(changes.desc), inverted.appendSet(lastItem.inverted), lastItem.selection)) : branch;
	    }
	  ...
	  function popChanges(branch, only) {
	    ...
	    for (;; idx--) {
	      ...
	      if (entry.isChange || only == 1
	      /* Any */
	      && entry.selection) break;
	      map = map ? entry.map.appendSet(map) : entry.map;
	    }

	    var changeItem = branch[idx];
	    var newBranch = branch.slice(0, idx),
	        changes = changeItem.inverted || src$1.ChangeSet.empty,
	        selection = changeItem.selection;
	  ...
	  function () {
	    ...
	    HistoryState.prototype.addChanges = function (changes, inverted, selection, time, userEvent, newGroupDelay, maxLen) {
	      var mayMerge = nope;
	      if (this.prevTime !== null && time - this.prevTime < newGroupDelay && (inverted || this.prevUserEvent == userEvent && userEvent == "keyboard")) mayMerge = inverted ? function (prev) {
	        return isAdjacent(prev.map.changes[prev.map.length - 1], changes.changes[0]);
	      } : function (prev) {
	        return eqSelectionShape(prev.selection, selection);
	      };
	    ...
	    HistoryState.prototype.pop = function (done, only, transaction, maxLen) {
	      var _a = popChanges(done == 0
	      /* Done */
	      ? this.done : this.undone, only),
	          changes = _a.changes,
	          branch = _a.branch,
	          selection = _a.selection;
	      ...
	      var otherBranch = done == 0
	      /* Done */
	      ? this.undone : this.done;
	      ...
	      return {
	        ...
	        state: new HistoryState(done == 0
	        /* Done */
	        ? branch : otherBranch, done == 0
	        /* Done */
	        ? otherBranch : branch)
	      };
	    };
	...
	var history = createCommonjsModule(function (module, exports) {
	  ...
	  function historyField(_a) {
	    var minDepth = _a.minDepth,
	        newGroupDelay = _a.newGroupDelay;
	    return new src$1.StateField({
	      ...
	      apply: function (tr, state, editorState) {
	        ...
	        if (tr.getMeta(exports.closeHistorySlot)) state = state.resetTime();
	        if (!tr.changes.length && !tr.selectionSet) return state;
	        if (tr.getMeta(src$1.Transaction.addToHistory) !== false) return state.addChanges(tr.changes, tr.changes.length ? tr.invertedChanges() : null, tr.startState.selection, tr.getMeta(src$1.Transaction.time), tr.getMeta(src$1.Transaction.userEvent), newGroupDelay, minDepth);
	  ...
	  function cmd(target, only) {
	    return function (_a) {
	      ...
	      var _c = historyState.pop(target, only, state.t(), config.minDepth),
	          transaction = _c.transaction,
	          newState = _c.state;
	  ...
	  exports.undoSelection = cmd(0
	  /* Done */
	  , 1
	  /* Any */
	  );
	  exports.redoSelection = cmd(1
	  /* Undone */
	  , 1
	  /* Any */
	  ...
	  exports.undoDepth = depth(0
	  /* Done */
	  , 0
	  /* OnlyChanges */
	  ); // The amount of redoable change events available in a given state.

	  exports.redoDepth = depth(1
	  /* Undone */
	  , 0
	  /* OnlyChanges */
	  ); // The amount of undoable events available in a given state.

	  exports.redoSelectionDepth = depth(0
	  /* Done */
	  , 1
	  /* Any */
	  ); // The amount of redoable events available in a given state.

	  exports.undoSelectionDepth = depth(1
	  /* Undone */
	  , 1
	  /* Any */
	  );
	...
	var src$3 = createCommonjsModule(function (module, exports) {
	  ...
	  function (_super) {
	    ...
	    GutterMarker.prototype.toDOM = function () {
	      return null;
	    };
	  ...
	  function gutter(config) {
	    ...
	    return src$2.ViewExtension.all(src$2.viewPlugin(function (view) {
	      return new GutterView(view, conf);
	  ...
	  function () {
	    ...
	    GutterView.prototype.updateGutter = function () {
	      ...
	      this.view.viewportLines(function (line) {
	        var text;
	        if (Array.isArray(line.type)) text = line.type.find(function (b) {
	          return b.type == 0
	          /* Text */
	          ;
	        });else text = line.type == 0
	        /* Text */
	        ? line : undefined;
	        if (!text) return;
	        ...
	        if (localMarkers.length || _this.config.renderEmptyElements) {
	          ...
	          if (i == _this.elements.length) {
	            ...
	            var markers_1 = localMarkers,
	                elt = _this.elements[i];
	  ...
	  function () {
	    ...
	    GutterElement.prototype.update = function (height, above, markers, eltClass) {
	      ...
	      if (this.markers != markers) {
	        ...
	        for (var ch = void 0; ch = this.dom.lastChild;) ch.remove();
	...
	var commands = createCommonjsModule(function (module, exports) {
	  ...
	  function moveSelection(view, dir, granularity) {
	    var transaction = view.state.t().forEachRange(function (range) {
	      if (!range.empty && granularity != "lineboundary") return new src$1.SelectionRange(dir == "left" || dir == "backward" ? range.from : range.to);
	    ...
	    if (granularity == "line") transaction.addMeta(src$1.Transaction.preserveGoalColumn(true));
	    view.dispatch(transaction.scrollIntoView());
	    return true;
	  }

	  exports.moveCharLeft = function (view) {
	    return moveSelection(view, "left", "character");
	  };

	  exports.moveCharRight = function (view) {
	    return moveSelection(view, "right", "character");
	  };

	  exports.moveWordLeft = function (view) {
	    return moveSelection(view, "left", "word");
	  };

	  exports.moveWordRight = function (view) {
	    return moveSelection(view, "right", "word");
	  };

	  exports.moveLineUp = function (view) {
	    return moveSelection(view, "backward", "line");
	  };

	  exports.moveLineDown = function (view) {
	    return moveSelection(view, "forward", "line");
	  };

	  exports.moveLineStart = function (view) {
	    return moveSelection(view, "backward", "lineboundary");
	  };

	  exports.moveLineEnd = function (view) {
	    return moveSelection(view, "forward", "lineboundary");
	  };
	  ...
	  exports.extendCharLeft = function (view) {
	    return extendSelection(view, "left", "character");
	  };

	  exports.extendCharRight = function (view) {
	    return extendSelection(view, "right", "character");
	  };

	  exports.extendWordLeft = function (view) {
	    return extendSelection(view, "left", "word");
	  };

	  exports.extendWordRight = function (view) {
	    return extendSelection(view, "right", "word");
	  };

	  exports.extendLineUp = function (view) {
	    return extendSelection(view, "backward", "line");
	  };

	  exports.extendLineDown = function (view) {
	    return extendSelection(view, "forward", "line");
	  };

	  exports.extendLineStart = function (view) {
	    return extendSelection(view, "backward", "lineboundary");
	  };

	  exports.extendLineEnd = function (view) {
	    return extendSelection(view, "forward", "lineboundary");
	  };
	  ...
	  exports.deleteCharBackward = function (view) {
	    return deleteText(view, "backward");
	  };

	  exports.deleteCharForward = function (view) {
	    return deleteText(view, "forward");
	  ...
	  function insertNewlineAndIndent(_a) {
	    var state = _a.state,
	        dispatch = _a.dispatch;
	    var i = 0,
	        indentation = state.selection.ranges.map(function (r) {
	  ...
	  function indentSelection(_a) {
	    ...
	    for (var _i = 0, _c = state.selection.ranges; _i < _c.length; _i++) {
	      ...
	      for (var _d = state.doc.lineAt(range.from), start = _d.start, end = _d.end;;) {
	        if (start != lastLine) {
	          lastLine = start;
	          var indent = getIndentation(state, start),
	              current = void 0;
	          if (indent > -1 && indent != (current = /^\s*/.exec(state.doc.slice(start, Math.min(end, start + 100)))[0].length)) positions.push({
	            pos: start,
	            current: current,
	            indent: indent
	          });
	  ...
	  for (var key in exports.pcBaseKeymap) exports.macBaseKeymap[key] = exports.pcBaseKeymap[key];
	...
	var stringstream = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    StringStream.prototype.indentation = function () {
	      return misc.countColumn(this.string, null, this.tabSize) - (this.lineStart ? misc.countColumn(this.string, this.lineStart, this.tabSize) : 0);
	    };
	...
	var src$4 = createCommonjsModule(function (module, exports) {
	  ...
	  function () {
	    ...
	    CachedState.prototype.copy = function (mode) {
	      return new CachedState(util.copyState(mode, this.state), this.pos);
	    };
	  ...
	  function cutDecoratedRange(range, at) {
	    ...
	    return {
	      from: range.from,
	      to: Math.min(at, range.to),
	      decorations: range.decorations.filter(function (_a) {
	        var to = _a.to;
	        return to <= at;
	      })
	    };
	  ...
	  function () {
	      ...
	      return new Promise(function (resolve) {
	        var f = function () {
	          ...
	          do {
	            ...
	            if (_this.frontier >= to) return resolve();
	    ...
	    StateCache.prototype.calculateDecorations = function (editorState, from, to, mode) {
	      ...
	      for (var i = 0; cursor.offset + stream.start < to;) {
	        if (stream.eol()) {
	          stream = cursor.next();
	          if (++i % 5 == 0) states.push(new CachedState(util.copyState(mode, state), cursor.offset));
	        } else {
	          var style = util.readToken(mode, stream, state);
	          if (style) decorations.push(decoration.Decoration.mark(cursor.offset + stream.start, cursor.offset + stream.pos, {
	    ...
	    StateCache.prototype.storeStates = function (from, to, states) {
	      ...
	      var start = this.findIndex(from),
	          end = this.findIndex(to);
	    ...
	    StateCache.prototype.getState = function (editorState, pos, mode) {
	      var _a = this.stateBefore(pos, mode),
	          statePos = _a.pos,
	          state = _a.state;
	      ...
	      } else if (this.lastDecorations && statePos < this.lastDecorations.from && this.lastDecorations.from <= pos) // If we are calculating a correct state for a position that is after the
	      ...
	      if (statePos < pos) {
	        ...
	        while (statePos < pos) {
	          if (stream.eol()) {
	            stream = cursor.next();
	            statePos++;
	            if (++i % 50) states.push(new CachedState(util.copyState(mode, state), statePos));
	          } else {
	    ...
	    StateCache.prototype.apply = function (transaction) {
	      ...
	      var start = transaction.doc.lineAt(transaction.changes.changes.reduce(function (m, ch) {
	        return Math.min(m, ch.from);
	      }, 1e9)).start;
	      var states = [];
	  ...
	  exports.legacyMode = function (config) {
	    var field = new src$1.StateField({
	      init: function (state) {
	        return new StateCache([], 0, null);
	      },
	      apply: function (tr, cache) {
	        return cache.apply(tr);
	      }
	    });
	  ...
	  function decoSpec(field, config) {
	    var _a = config.sleepTime,
	        sleepTime = _a === void 0 ? 100 : _a,
	        _b = config.maxWorkTime,
	        maxWorkTime = _b === void 0 ? 100 : _b,
	        mode = config.mode;
	    var decorations = decoration.Decoration.none,
	        from = -1,
	        to = -1;
	    ...
	    return {
	      ...
	      update: function (_, u) {
	        return update(u.view, u.docChanged);
	      }
	    };
	  }
	});
	...
	var matchbrackets = createCommonjsModule(function (module, exports) {
	  ...
	  function findMatchingBracket(doc, where, config) {
	    var pos = where - 1; // A cursor is defined as between two characters, but in in vim command mode
	    // (i.e. not insert mode), the cursor is visually represented as a
	    // highlighted box on top of the 2nd character. Otherwise, we allow matches
	    // from before or after the cursor.

	    var match = !config.afterCursor && pos >= 0 && matching[doc.slice(pos, pos + 1)] || matching[doc.slice(++pos, pos + 1)];
	  ...
	  function doMatchBrackets(state, config) {
	    ...
	    for (var _i = 0, _a = state.selection.ranges; _i < _a.length; _i++) {
	      ...
	      decorations.push(decoration.Decoration.mark(match.from, match.from + 1, {
	        class: style
	      }));
	      if (match.to) decorations.push(decoration.Decoration.mark(match.to, match.to + 1, {
	        class: style
	      }));
	    }
	  ...
	  return {
	    ...
	    indent: function (state, textAfter) {
	      ...
	      var firstChar = textAfter && textAfter.charAt(0),
	          ...
	          top; // Kludge to prevent 'maybeelse' from blocking lexical scope pops
	...
	*/
	...
	var specialChars = createCommonjsModule(function (module, exports) {
	  ...
	  exports.specialChars = src$2.ViewExtension.unique(function (configs) {
	    ...
	    return new src$2.ViewField({
	      create: function (view) {
	        return new SpecialCharHighlighter(view, config);
	      },
	      update: function (self, update) {
	        return self.update(update);
	      },
	      effects: [src$2.ViewField.decorationEffect(function (self) {
	  ...
	  function () {
	    function SpecialCharHighlighter(view, options) {
	      ...
	      if (this.replaceTabs = (styles.tabSize || styles.MozTabSize) == null) this.specials = new RegExp("\t|" + this.specials.source, "gu");
	    }

	    SpecialCharHighlighter.prototype.update = function (update) {
	      if (this.replaceTabs && update.transactions.some(function (tr) {
	        return tr.getMeta(src$1.Transaction.changeTabSize) != null;
	      })) {
	        this.decorations = src$2.Decoration.none;
	        this.from = this.to = 0;
	      } else {
	    ...
	    SpecialCharHighlighter.prototype.closeHoles = function (ranges) {
	      var decorations = [],
	          vp = this.view.viewport,
	          replaced = [];

	      for (var i = 0; i < ranges.length; i++) {
	        var _a = ranges[i],
	            from = _a.fromB,
	            to = _a.toB; // Must redraw all tabs further on the line

	        if (this.replaceTabs) to = this.view.state.doc.lineAt(to).end;

	        while (i < ranges.length - 1 && ranges[i + 1].fromB < to + JOIN_GAP) to = Math.max(to, ranges[++i].toB); // Clip to current viewport, to avoid doing work for invisible text
	      ...
	      if (decorations.length) this.decorations = this.decorations.update(decorations, function (pos) {
	        for (var i = 0; i < replaced.length; i += 2) if (pos >= replaced[i] && pos < replaced[i + 1]) return false;
	    ...
	    SpecialCharHighlighter.prototype.updateForViewport = function () {
	      var vp = this.view.viewport; // Viewports match, don't do anything
	      ...
	      if (this.from >= vp.to || this.to <= vp.from) {
	        ...
	        this.decorations = this.decorations.update(decorations, function (from, to) {
	      ...
	      for (var pos = from, cursor = doc.iterRange(from, to), m = void 0; !cursor.next().done;) {
	        if (!cursor.lineBreak) {
	          while (m = SPECIALS.exec(cursor.value)) {
	            var code = m[0].codePointAt ? m[0].codePointAt(0) : m[0].charCodeAt(0),
	                widget = void 0;
	            if (code == null) continue;

	            if (code == 9) {
	              var line = doc.lineAt(pos + m.index);
	              var size = this.view.state.tabSize,
	                  col = src.countColumn(doc.slice(line.start, pos + m.index), 0, size);
	              widget = new TabWidget((size - col % size) * this.view.defaultCharacterWidth);
	            } else {
	              widget = new SpecialCharWidget(this.options, code);
	            }

	            target.push(src$2.Decoration.replace(pos + m.index, pos + m.index + m[0].length, {
	              widget: widget
	            }));
	          }
	        }
	  ...
	  function (_super) {
	    ...
	    SpecialCharWidget.prototype.ignoreEvent = function () {
	      return false;
	    };
	  ...
	  function (_super) {
	    ...
	    TabWidget.prototype.ignoreEvent = function () {
	      return false;
	    };
	...
	var multipleSelections = createCommonjsModule(function (module, exports) {
	  ...
	  exports.multipleSelections = src$1.StateExtension.unique(function (configs) {
	    var rangeConfig = {
	      class: styles.secondarySelection
	    }; // FIXME configurable?

	    return src$1.StateExtension.all(src$1.StateExtension.allowMultipleSelections(true), src$2.ViewField.decorations({
	      create: function (view) {
	        return decorateSelections(view.state, rangeConfig);
	      },
	      update: function (deco, _a) {
	        var prevState = _a.prevState,
	            state = _a.state;
	        return prevState.doc == state.doc && prevState.selection.eq(state.selection) ? deco : decorateSelections(state, rangeConfig);
	      }
	    }), src$2.styleModule(styles));
	  }, {});
	  ...
	  function decorateSelections(state, rangeConfig) {
	    var _a = state.selection,
	        ranges = _a.ranges,
	        primaryIndex = _a.primaryIndex;
	    if (ranges.length == 1) return src$2.Decoration.none;
	    var deco = [];

	    for (var i = 0; i < ranges.length; i++) if (i != primaryIndex) {
	      var range = ranges[i];
	      deco.push(range.empty ? src$2.Decoration.widget(range.from, {
	...
	function () {
	  function Completion(value) {
	    ...
	    this.tokenize(value).then(function (tokens) {
	      _this.set = new Set(tokens);
	    });
	  }

	  var _proto = Completion.prototype;

	  _proto.tokenize = function tokenize(str) {
	    return new Promise(function (resolve, reject) {
	      resolve(str.replace(/[^\w\d\s]/g, "").replace(/\s{2,}/g, " ").split("\n").join(" ").split(" "));
	    });
	  };
	  ...
	  /*#__PURE__*/
	  function () {
	    var _appendToSet = _asyncToGenerator(
	    /*#__PURE__*/
	    ...
	    function appendToSet(_x) {
	      return _appendToSet.apply(this, arguments);
	    }

	    return appendToSet;
	  }();

	  _proto.getSuggestions = function getSuggestions(currentWord, content) {
	    if (content === void 0) {
	      content = null;
	    }
	    ...
	    this.set.forEach(function (token) {
	      // var score = levenshtein(currentWord, token)
	      var score = token.indexOf(currentWord) != -1 ? 1 : -1;
	  ...
	  _proto.getLastToken = function getLastToken() {
	...
	/*#__PURE__*/
	function () {
	  function editor(ext, value, settings) {
	    var _this = this;
	    ...
	    if (ext == null && value == null) {
	      ...
	      script.onload = function () {
	        ...
	        _this.cm = EditorState.create({
	          doc: value,
	          extensions: [lineNumbers(), history$1(), specialChars$1(), multipleSelections$1(), mode, matchBrackets(), keymap$1({
	            "Mod-z": undo,
	            "Mod-Shift-z": redo,
	            "Mod-u": function ModU(view) {
	        ...
	        _this.listenForAutomcompletion();
	      };

	      script.src = "mode/" + mode.mode + "/" + mode.mode + ".js";
	      document.head.appendChild(script); // after settings
	  ...
	  _proto.loadTheme = function loadTheme(theme) {
	  ...
	  _proto.fontSize = function fontSize(v) {
	    if (v > 0) {
	  ...
	  _proto.getLangName = function getLangName() {
	    return this.mode.name;
	  };

	  _proto.insertSnippet = function insertSnippet(snippet, replaceLine) {
	    if (replaceLine === void 0) {
	      replaceLine = false;
	    }
	  ...
	  _proto.listenForAutomcompletion = function listenForAutomcompletion() {
	    var _this3 = this;
	    ...
	    document.querySelector(".codemirror-content").addEventListener("input", function (e) {
	      var currentWord = _this3.c.getLastToken();

	      var suggestions = _this3.c.getSuggestions(currentWord, window.view.state.doc.toString());

	      _this3.setCompletion.apply(_this3, suggestions);
	    });
	  };

	  _proto.setCompletion = function setCompletion(a, b, c) {
	    window.webkit.messageHandlers.completion.postMessage([a, b, c]);
	  };

	  _proto.registerPlugin = function registerPlugin(obj, type) {
